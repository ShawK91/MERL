...
from envs.madrl import AbstractMAEnv, Agent, EzPickle
...
class AntLeg(Agent):
    ...
    def get_observation(self):
        ...
        return np.concatenate([
            ...
            np.random.normal(self.qvel.flat[:6], self.vel_noise), # body vel
            np.random.normal(self.qpos.flat[7+2*idx:9+2*idx], self.pos_noise),
            np.random.normal(self.qvel.flat[6+2*idx:8+2*idx], self.vel_noise),
            np.random.normal(self.qpos.flat[7+2*n1_idx:9+2*n1_idx], self.pos_noise),
            np.random.normal(self.qvel.flat[6+2*n1_idx:8+2*n1_idx], self.vel_noise),
            np.random.normal(self.qpos.flat[7+2*n2_idx:9+2*n2_idx], self.pos_noise),
            np.random.normal(self.qvel.flat[6+2*n2_idx:8+2*n2_idx], self.vel_noise),
            np.random.normal(np.clip(self.data.cfrc_ext[3*idx+2:3*idx+5], -1, 1).flat, self.force_noise)
        ])
