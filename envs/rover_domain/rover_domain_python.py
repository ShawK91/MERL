...
class RoverDomainOverActuated:
	...
	def step(self, joint_action):
		...
		for rover_id in range(self.args.num_agents):
			...
			mag = np.mean(action[::2])
			bearing = np.mean(action[1:][::2])


			magnitude = 0.5*(mag+1) # [-1,1] --> [0,1]

			bearing /= 2.0 #Theta (bearing constrained to be within 90 degree turn from heading)
			theta = bearing * 180 + self.rover_pos[rover_id][2]
			if theta > 360: theta -= 360
			if theta < 0: theta += 360
			self.rover_pos[rover_id][2] = theta
			...
			#Log
			self.rover_path[rover_id].append((self.rover_pos[rover_id][0], self.rover_pos[rover_id][1], self.rover_pos[rover_id][2]))
			self.action_seq[rover_id].append([magnitude, bearing*180])
